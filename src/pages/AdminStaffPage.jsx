  ...
  useEffect(() => {
    const fetchAdminStaff = async () => {
      try {
        ...
        if (res.data.success) {
          console.log('Admin staff fetched:', res.data.data.length);
          setAdminStaff(res.data.data);
        }
      } catch (err) {
        console.error('Error fetching administrative staff:', err);
        setError('Failed to load administrative staff. Please try again later.');
      } finally {
        setLoading(false);
      }
    };

    console.log('Component mounted - fetching admin staff');
    fetchAdminStaff();
  }, []);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this staff member? This action cannot be undone.')) {
      try {
        ...
        setAdminStaff(adminStaff.filter(staff => staff._id !== id));
      } catch (err) {
        console.error('Error deleting administrative staff:', err);
        alert('Failed to delete administrative staff member');
      }
    }
  };

  // Get unique positions and departments for filter dropdowns
  const positions = [...new Set(adminStaff.map(staff => staff.position))];
  const departments = [...new Set(adminStaff.map(staff => staff.department))];

  // Filter admin staff based on search term, position filter, and department filter
  const getFilteredStaff = () => {
    return adminStaff.filter(staff => {
      const matchesSearch =
                            ...
                            <div className="flex-shrink-0 h-10 w-10 rounded-full overflow-hidden">
                              {staff.user?.profileImage?.url ? (
                                <img
                                  ...
                                  alt={staff.user?.name || 'Admin'}
                                  className="h-full w-full object-cover"
                                  onError={(e) => {
                                    e.target.onerror = null;
                                    e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(staff.user?.name?.charAt(0) || 'A')}&background=0D8ABC&color=fff&size=256`;
                                  }}
                                />
                              ) : (
                                <div className="h-full w-full bg-indigo-500 flex items-center justify-center text-white font-bold">
                                  {staff.user?.name?.charAt(0) || 'A'}
                                </div>
                              )}
                            </div>
                            <div className="ml-4">
                              ...
                              <div className="text-sm text-gray-500">
                                {staff.position.charAt(0).toUpperCase() + staff.position.slice(1)} | {staff.department}
                              </div>
                            </div>
                          </div>
