...
const ReceiptUpload = ({ onDataExtracted, studentInfo }) => {
  ...
  const validateAndSetFile = (selectedFile) => {
    ...
    let filename = 'fee_receipt';

    // Add student info if available
    if (studentInfo) {
      if (studentInfo.name) {
        filename += `_${studentInfo.name.toLowerCase().replace(/\s+/g, '_')}`;
      }
      if (studentInfo.class) {
        filename += `_class_${studentInfo.class}`;
      }
      if (studentInfo.section) {
        filename += `_section_${studentInfo.section}`;
      }
      if (studentInfo.rollNumber) {
        filename += `_roll_${studentInfo.rollNumber}`;
      }
    }
    ...
    const renamedFileObj = new File([selectedFile], filename, {
    ...
    console.log('File selected and renamed for better data extraction:', filename);
  };
