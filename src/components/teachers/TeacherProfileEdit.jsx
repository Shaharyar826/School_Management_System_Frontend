...
const TeacherProfileEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        console.log('Starting to fetch data for TeacherProfileEdit component');
        ...
        try {
          ...
          if (classesRes.data.success && classesRes.data.data.length > 0) {
            ...
            console.log('Available classes fetched from database:', classes);
        ...
        if (teacherRes.data.success) {
          ...
          console.log('Teacher classes from API:', teacher.classes);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use the profile endpoint for teachers
      const res = await axios.put('/api/teachers/profile', {
        userData,
        teacherData
      });
