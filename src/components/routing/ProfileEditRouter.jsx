import { useContext } from 'react';
import AuthContext from '../../context/AuthContext';
import TeacherProfileEdit from '../teachers/TeacherProfileEdit';
import AdminProfileEdit from '../admin/AdminProfileEdit';
import StudentProfileEdit from '../students/StudentProfileEdit';

const ProfileEditRouter = () => {
  const { user } = useContext(AuthContext);

  // Render the appropriate profile edit component based on user role
  if (user && (user.role === 'admin' || user.role === 'principal')) {
    return <AdminProfileEdit />;
  }

  if (user && user.role === 'teacher') {
    return <TeacherProfileEdit />;
  }

  if (user && user.role === 'student') {
    return <StudentProfileEdit />;
  }

  // Default fallback - redirect to user profile page for other roles
  return <div className="py-6">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4">
        <div className="flex">
          <div className="flex-shrink-0">
            <svg className="h-5 w-5 text-yellow-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
          </div>
          <div className="ml-3">
            <p className="text-sm text-yellow-700">Profile editing not available for your role. Please use the Profile Settings page.</p>
          </div>
        </div>
      </div>
    </div>
  </div>;
};

export default ProfileEditRouter;
